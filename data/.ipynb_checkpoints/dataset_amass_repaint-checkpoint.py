...
class AMASS_Dataset(Dataset):
    ...
    def __init__(self, opt):
        ...
        # Contact dataset selection
        filenames_test = os.path.join(dataroot, '*/test/*.pkl')
        # filenames_test = os.path.join(dataroot, '*/train/*.pkl')
...
#            self.filename_list = glob.glob('data_fps60/*/test/*.pkl')
            self.filename_list = sorted(glob.glob(filenames_test))
    ...
    def __getitem__(self, idx):
        ...
        if self.opt['phase'] == 'train':
            ...
            p_uncond = 0.2
            unc = np.random.choice([0, 1], p=[p_uncond, 1 - p_uncond])
            if unc == 0:
                input_hmd[:,39:45] = 0.          # Allow head
                input_hmd[:,48:] = 0.          # Allow head
            ...
            input_hmd[:,36:] = input_hmd[:,36:]
            output_gt = rotation_local_full_gt_list[1:]
            # print('ip hmd = ' + str(input_hmd.shape))
...
class Conditioned_AMASS_Dataset(AMASS_Dataset):
    def __init__(self, opt):
        super().__init__(opt)
