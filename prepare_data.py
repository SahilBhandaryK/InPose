...
def main(phase):
    ...
    if phase == 'test':
        datasets = ["Transitions_mocap", "HumanEva"]
    elif phase == 'train':
        datasets = ["ACCAD", "BioMotionLab_NTroje", "BMLhandball", "BMLmovi", "CMU", "KIT", "MPI_HDM05", "MPI_Limits", "SFU", "TCD_handMocap"]
    
    for dataroot_subset in datasets:
        print(dataroot_subset)
        savedir = os.path.join("./data_fps60", dataroot_subset, phase)
        if not os.path.exists(savedir):
            os.makedirs(savedir)

        rootdir = './data/dataset_raw/AMASS/' + dataroot_subset + '/'
        regex = '*/*.npz'
        
        filepaths = glob.glob(os.path.join(rootdir, regex))
        ...
        #            self.rotation_local_full_gt_list.append(torch.Tensor(bdata['poses'][:, :66]))
            ...
            if phase == 'train':
                data['rotation_local_full_gt_list'] = rotation_global_6d[1:,:22].reshape(-1,132)	# global
            elif phase == 'test':
                data['rotation_local_full_gt_list'] = rotation_local_full_gt_list	# local
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Data Preperation, please download AMASS as described in the README.")
    
    parser.add_argument("-d", "--data_type", default="test", help="Choose either train/test")
