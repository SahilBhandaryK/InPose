...
def global2local_pose(global_pose, kintree):
    bs = global_pose.shape[0]

    global_pose = global_pose.view(bs, -1, 3, 3)

    local_pose = global_pose.clone()

    for jId in range(1,len(kintree)):
        parent_id = kintree[jId]
        local_pose[:, jId] = torch.matmul(torch.transpose(global_pose[:, parent_id],dim0=1,dim1=2), global_pose[:, jId])
